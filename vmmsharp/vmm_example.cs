...
class vmm_example
{
    ...
    static void Main(string[] args)
    {
        ...
        var arr = Encoding.Unicode.GetBytes("我");


        VmmH v = new VmmH(5284, "TslGame.exe");
        var m = v.GetModuleFromName();
        //var t  = v.ReadUL(m.vaBase);

        //解密
        var func = v.ReadUL(m.vaBase + 0x07AE2528);
        var offset = v.ReadUI(func + 3)+7;
        var ptr = func + offset;
        var arrByte = v.Read(func + 7, 1024);

        var s = "";
        foreach (var item in arrByte)
        {
            s += item.ToString() + ",";
        }
        Console.WriteLine(s);
        ...
        //世界
        var GWorld = v.ReadUL(m.vaBase + 0x090F2FD0);
        Console.ReadLine();


        //var m = v.GetModuleFromName("Conquer.exe");
        //var worldBase = v.ReadStrUni(m.vaBase + 0x0ADC0824);
